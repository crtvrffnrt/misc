...
$targetWidth = 5120
$targetHeight = 2160
$preferredRefreshRates = @(120.0, 119.99, 119.98, 119.94)
...
Write-Info "Requested mode: ${targetWidth}x${targetHeight}"
Write-Info "Refresh preference order: $($preferredRefreshRates -join ', ') Hz"

try {
    Write-Info "Applying native resolution ${targetWidth}x${targetHeight}..."
    Set-DisplayResolution -DisplayId $selectedDisplay.DisplayId -Width $targetWidth -Height $targetHeight -AllowChanges -ErrorAction Stop | Out-Null
    ...
    if ($stateAfterResolution.Width -ne $targetWidth -or $stateAfterResolution.Height -ne $targetHeight) {
        throw "Resolution verification failed. Current mode is '$($stateAfterResolution.Mode)' instead of ${targetWidth}x${targetHeight}."
    }

    Write-Info 'Restoring Windows recommended DPI scaling for the native panel mode...'
    Set-DisplayScale -DisplayId $selectedDisplay.DisplayId -Recommended -ErrorAction Stop

    $appliedRefreshRate = Set-DisplayModeWithRefreshFallback -TargetDisplayId $selectedDisplay.DisplayId -Width $targetWidth -Height $targetHeight -RefreshRates $preferredRefreshRates
    ...
    if ($finalState.Width -ne $targetWidth -or $finalState.Height -ne $targetHeight) {
        throw "Final verification failed because the resolution is '$($finalState.Mode)' instead of ${targetWidth}x${targetHeight}."
    }
    ...
    if (-not $refreshMatched) {
        throw "Final verification failed because the refresh rate is '$($finalState.RefreshHz)' Hz instead of a value near 120 Hz. Current mode: '$($finalState.Mode)'."
    }

    Write-Ok "Applied native mode $($finalState.Mode) to $displayLabel."
}
catch {
